# Contributing to the Astir project

Hello possible new developer, and welcome to the Astir project!
...
* You can ask and answer questions on the Astir mailing list
* You can report bugs
* You can fix bugs
* You can add or modify features in Astir
...
Github Discussions at https://github.com/queueingqt/Astir/discussions .

### Please don't open a Github Issue for questions about the code

Github's "Issue" tracker is for reporting bugs.

We would prefer it if you would not use Astir's git hub issue tracker
just to ask questions, as this is not the ideal forum and doesn't
reach the full Astir user community.  Your best bet is to get your
...
Once you're confident enough with Astir to be answering users'
...
### Adding or modifying features in Astir

Maybe Astir is lacking some feature you really want, or you really
would like to make some other feature of Astir work differently.  The
...
* Read
[Coding Standard](https://github.com/queueingqt/Astir/wiki/Coding-Standard).
...
* Check the Astir issue tracker: https://github.com/queueingqt/Astir/issues
...
* Check with the Astir-Dev list first to see if anyone else is
working on that particular idea or section of code.

* Verify on the Astir-Dev list that your idea has some merit and
...
* Keep in mind that Astir runs on multiple operating systems, so
code for that.  Some #ifdef's may be required in order to make it
work for the various operating systems.

* Astir can be run in multiple languages, so code for that.  If
...
result to Astir developers, this is no longer a desirable way to
...
* Log in to Astir's github repo at
  https://github.com/queueingqt/Astir.git and click the "Fork" button.
...
* Clone your repo:
      ...
      cd <some place other than where your other Astir clone lives>
      git clone https://github.com/youruserid/Astir.git
      cd Astir
  ...
  checked out the default branch, which in the case of the Astir repo
  is just the "master" branch.

* Add the official repo as a second remote called "upstream":

      git remote add upstream https://github.com/queueingqt/Astir.git
...
* When you have finished your work (or when you think you have) and are
  ...
  * A good practice at this point, before you've asked the Astir developers
    ...
    run all of Astir's Github Actions, checking that your new code
  ...
  * Now you are ready to ask for your code to be reviewed and accepted.
    ...
    * Open your web browser to your GitHub repo for your fork at
      https://github.com/youruserid/Astir.git.
    ...
    * A member of the Astir project will review your changes and
...
This section contains some simple ways for Astir users and developers
to check out code in other people's repos with a minimum of fuss.

Say user "sumgai" has a topic branch named "sumnufeetyur" in a fork of Astir
at "https://github.com/sumgai/Astir.git".  You want to pull those changes
...
* In your regular Astir clone directory, create a new branch to hold
...
* Now pull sumgai's branch into yours:
      ...
      git pull https://github.com/sumgai/Astir.git sumnufeetuyr
...
* When you're done testing sumgai's code, you can go back to the
  unmodified Astir just by doing "git checkout master," and
...
Astir is a multi-threaded and multi-process application.  It uses
...
### Old notes on using GDB with Astir
...
running astir inside gdb instead of at the command prompt ---
...
If you send the output of those three commands to the astir-dev
...
reversed *[Ed. Note: from what it was in earlier days of Astir]*.
"astir -?" should show the change once you compile it.
This means that we'll do core dumps by default upon segfault instead
of using the internal Astir segfault handler.  We've also
...
setting Astir SUID (if you're not using AX.25 kernel networking
...
Now when I do "kill -11 <pid>" I get this:
    ...
    [1]+  Segmentation fault      (core dumped) astir
...
worth, if Astir has SUID permission bit set (4755) I don't get a
core dump, but if it is reset I do (0755).

I can invoke ddd like this (it uses gdb under the hood):

    ddd /usr/local/bin/astir core.7586 &

or gdb like this:

    gdb -c core.7586 /usr/local/bin/astir
...
don't expect.  I just had one appear in ~/.astir/tmp, so if you
think you should have a core file and can't find it, try:

    find . -type f | grep core

The core file may be written to a directory that Astir is currently
in, instead of where you started Astir from.

### Customized debugging builds

Sometimes it is helpful to build astir with specialized compiler
...
The automake/autoconf setup of astir makes it easy to maintain several
...
point forward, you don't build astir in the source code directory
anymore.
...
Astir tree instead.  It doesn't matter, but does change how you
...
The beauty of this is you can make a second build without doing a make clean:
    ...
    mkdir -p build-debug
    cd build-debug
    ../Astir/configure --program-suffix="-debug" CFLAGS="-O -g -fno-inline"
    make && sudo  make install

This will build a second binary called "astir-debug" and install it,
